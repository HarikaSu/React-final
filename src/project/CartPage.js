...
const CartPage = () => {
  ...
  const [cart, setCart] = React.useState(
    JSON.parse(localStorage.getItem("cart")) || location.state?.cart || []
  );
  const [isMenuOpen, setIsMenuOpen] = React.useState(false);
  ...
  const calculateTotal = () => {
    return cart.reduce((total, item) => total + item.Price.INR * item.quantity, 0).toFixed(2);
  };
  ...
  return (
    <div>
      {/* Navbar */}
      <nav className="navbar navbar-expand-lg navbar-dark bg-dark shadow-sm px-4 py-3">
        <span className="navbar-brand fw-bold fs-4" onClick={() => navigate("/home")} style={{ cursor: "pointer" }}>
          <FaGem className="me-2" /> Bhavii Jeweller's
        </span>
        ...
        <div className={`collapse navbar-collapse ${isMenuOpen ? "show" : ""}`}>
          <ul className="navbar-nav ms-auto d-lg-flex align-items-center">
            <li className="nav-item">
              <button className="btn btn-outline-light px-3 me-3" onClick={() => navigate("/cart")}> 
                <FaShoppingCart className="me-2" /> Cart ({cart.reduce((acc, item) => acc + item.quantity, 0)})
              </button>
            </li>
            <li className="nav-item">
      ...
      <div className="cart-container">
        <div className="cart-card">
          ...
          <div className="cart-body">
            {cart.length === 0 ? (
              <div className="empty-cart-container">
                <img src="https://static.vecteezy.com/system/resources/previews/004/964/514/non_2x/young-man-shopping-push-empty-shopping-trolley-free-vector.jpg" alt="Empty Cart" className="empty-cart-img" />
                <p className="empty-cart-message">Oops! Your cart is empty. Start shopping now.</p>
                <button className="cart-btn" onClick={() => navigate("/home")}>Start Shopping</button>
              </div>
            ) : (
              <>
                <ul className="cart-list">
          ...
          <div className="cart-footer">
            <button className="cart-btn" onClick={() => navigate("/home")}>Continue Shopping</button>
          </div>
        </div>
      </div>
    </div>
